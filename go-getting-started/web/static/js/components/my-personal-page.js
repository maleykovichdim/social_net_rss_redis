...
export default async function renderPersonalPage() {
    ...
    const buttonAddPost = /** @type {HTMLButtonElement} */ (page.getElementById("add_post"))
    buttonAddPost.addEventListener("click", onAddPostFormSubmit)

    const buttonRSS = /** @type {HTMLButtonElement} */ (page.getElementById("rss"))
    buttonRSS.addEventListener("click", onRSSFormSubmit)
 ...
 * @param {Event} ev
 */
  async function onAddPostFormSubmit(ev) {
    ev.preventDefault() 
 ...
 * @param {Event} ev
 */
  async function onRSSFormSubmit(ev) {
    ev.preventDefault() 
    location.assign('/rss')
 }
